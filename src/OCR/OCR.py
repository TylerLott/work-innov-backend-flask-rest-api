...
class OCR:
    ...
    def __init__(self, image_path):
        ...
        self.bitnot, self.bounding_boxes = get_boxes(self.image)

    def get_bitnot_img(self):
        """ "return the image modified to not have table lines"""
        return self.bitnot
            ...
            for j in i:
                if len(j) == 0:
                    ...
                    for k in j:
                        ...
                        cropped_img = self.bitnot[x : x + h, y : y + w]
                        col = _run_tesseract(cropped_img)
                    row.append(col)

        arr = np.array(row)
        return arr.reshape(
            (len(self.bounding_boxes), len(self.bounding_boxes[0]), 2)
        ).tolist()
